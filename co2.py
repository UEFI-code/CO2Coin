...
def verifyTransaction(transaction):
    ...
    if sender == "0" * 192:
        if(len(jobs) == 0):
            print("No jobs to verify, but will give you mined reward!")
            return True
...
def verifyBlock(block):
    ...
    if len(chain) == 0:
        ...
        previousHash = chain[-1][1728:1792]
...
def makeBlock(transaction, miner, pow):
    if len(chain) == 0:
        previousHash = "0" * 64
    else:
        previousHash = chain[-1][1728:1792]
...
def makeWhiteMiningReward(receiver, pow):
    rewardTrans = makeTransaction("0" * 192, "0" * 192, 0, "!" * 320, SigningKey.generate(curve=NIST384p).to_string().hex())
    minedBlock = makeBlock(rewardTrans, receiver, pow)
...
def demo():
    ...
    res = makeWhiteMiningReward(vk1, 'p' * 512)
    if res:
    ...
    if trans != False:
        block = makeBlock(trans, vk2, 'p' * 512)
